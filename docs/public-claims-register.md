...
| Company positioning: "Trustworthy longitudinal brain MRI"; "We develop technology for detecting and assessing meaningful change between serial brain MRI examinations"; Lävi Delta is "our first product" | Hero h1 + standfirst; title/meta/OG | D6 + D8 (risk-owner instruction 2026-09-21) | DEVELOPMENT INTENT (positioning) | "Trustworthy" is a category/goal statement, not a performance claim — it must never sit next to an unqualified capability claim, and the hero status line + §12 footer must stay on the same screen | "We develop technology for…", never "solves", "eliminates", "guarantees" or "works on any scanner". "Intelligence layer" remains rejected ("intelligent" is banned register, §7). Delta's own description stays within the §2 intended-purpose ceiling |

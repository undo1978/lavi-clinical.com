...
| ~~Quality assessment produces three named graded outcomes: report / report with caution / explicit refusal~~ | — | §3.2 | **WITHDRAWN 2026-09-21 (D8 IP guardrail)** | A named output taxonomy is decision-logic disclosure | The QC strip and the three-grade naming were removed from `index.html` and `technology.html`. What remains is the behaviour only: the comparison is checked, and an untrustworthy one is withheld and the refusal stated |
...
| ~~Anatomical correspondence is never inferred from a method name; unsupported lanes abstain~~ | — | §3.3 | **WITHDRAWN 2026-09-21 (D8 IP guardrail)** | Describes when the system abstains, i.e. decision logic adjacent to the unpublished invention | Removed from `index.html` §06 and `technology.html` §03. Do not reinstate without patent counsel |
...
| Company positioning: "Not every difference is disease."; "We develop technology for detecting and assessing meaningful change between serial brain MRI examinations"; Lävi Delta is "our first product" | Hero h1 + standfirst; title/meta/OG | D6 + D8 (risk-owner instruction 2026-09-21) | VERIFIED FACT (headline is a clinical truth, not a product claim) + DEVELOPMENT INTENT (the "we develop" clause) | — | "Trustworthy" was drafted and **rejected 2026-09-21**: as an H1 it reads as a performance/quality claim about the product. The headline now states a fact about imaging, claiming nothing about Lävi. Never "solves", "eliminates", "guarantees", "works on any scanner"; "intelligence layer" stays rejected (§7) |
| Not every visible difference between two examinations is biological; scanner and protocol differences, acquisition variability, positioning and image processing all leave marks that can read as change; telling that apart from genuine biological change is the problem Lävi is built around | 01 Problem | §2 (problem statement) + D8 | VERIFIED FACT (clinical/physical background) + DEVELOPMENT INTENT (the "built around" clause) | Must state the *problem and goal only*. Any sentence describing how the distinction is made is IP-restricted (D8) and must not be published | As written. No mechanism, no calibration/normalisation/nuisance-modelling language, no decision logic, no measurement envelope |
